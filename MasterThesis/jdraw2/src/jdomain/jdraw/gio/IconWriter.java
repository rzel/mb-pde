...
public final class IconWriter {
   ...
   public static boolean writeIcon( Picture pic, String fileName ) {
      try {
         IconWriter writer = new IconWriter( new FileOutputStream( fileName ),
               pic );
         writer.writeIcon();
         return true;
   ...
   private void writeIconEntries() throws Exception {
      ...
      for ( int i = 0; i < frameCount; i++ ) {
         ...
         Log.debug( "icon #" + (i + 1) + " offset: " + offset );
   ...
   private final class IconEntry {
      ...
      public IconEntry( Frame aFrame ) throws Exception {
         ...
         if ( !hasAlpha ) {
            ...
            else if ( palSize <= Palette.GIF_MAX_COLOURS ) {
               bitsPerPixel = 8;
               div = 1;
            }
            else {
      ...
      private boolean hasAlpha() {
         ...
         final int count = pal.size();
         Color col;
         for ( int i = 0; i < count; i++ ) {
            col = pal.getColour( i ).getColour();
            if ( col.getAlpha() != Palette.MAX_RGB_VALUE ) {
               return true;
            }
         }
         return false;
      ...
      public void writeData() throws Exception {
         Assert.isTrue( offset == bytesWritten, "gio: wrong position."
         ...
         writeDWord( 0 );//numColours ); // number of colors in the image
         writeDWord( 0 );//numColours ); // minimum number of important colours
         ...
         if ( bitsPerPixel <= 8 ) {
            ...
            final int colourCount = hasTransColour ? numColours + 1
                  : numColours;
      ...
      public void createData() {
         ...
         switch ( bitsPerPixel ) {
            ...
            case 32:
               ...
               break;
            default:
               Assert.fail( "gio: cannot handle icons with bitsPerPixel="
                     + bitsPerPixel );
         }
      }

      public int getSize() {
         return ICON_HEADER_SIZE + bitMap.length + andMap.length
               + ((bitsPerPixel <= 8) ? numColours * 4 : 0);
      }
      ...
      private void createDataTrueColour24bit() {
         ...
         for ( int y = height - 1; y >= 0; y-- ) {
            for ( int x = 0; x < width; x++ ) {
               ...
               col = pal.getColour( pixel ).getColour();
               bitMap[index] = (byte) col.getBlue();
               bitMap[index + 1] = (byte) col.getGreen();
               bitMap[index + 2] = (byte) col.getRed();
               index += 3;
            }
         }
      }
         ...
         for ( int y = height - 1; y >= 0; y-- ) {
            for ( int x = 0; x < width; x++ ) {
               ...
               if ( pixel == transIndex ) {
                  ...
                  bitMap[index + 3] = (byte) col.getAlpha();
               }
               index += 4;
            }
         }
