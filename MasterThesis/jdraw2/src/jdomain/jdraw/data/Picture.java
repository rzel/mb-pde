...
public final class Picture extends DataObject {
   ...
   public static Picture createDefaultPicture() {
      Picture pic = createPicture( 32, 32 );
   ...
   protected void rebuildColourMaps() {
      ...
      final int size = pals.length;
      Log.debug( "Rebuilding colour maps..." );
      for ( int i = 0; i < size; i++ ) {
         pals[i].rebuildColourMap();
      }
   }
   ...
   public void scale( Dimension dimension, int scaleStyle ) {
      ...
      for ( int i = 0; i < frameCount; i++ ) {
         try {
            ...
            if ( PNGWriter.writePNG( f, out ) ) {
               ...
               icon = jdomain.util.ResourceLoader.scaleImage( icon, dimension, scaleStyle );
               scaledPic = ImageReader.readImage( icon );
               copy = scaledPic.getFrame( 0 );
               copy.setPalette( scaledPic.getPalette() );
               copy.setTransparent( scaledPic.getTransparent() );
               newFrameList.add( copy );
               copy.getSettings().setFrom( f.getSettings() );
            }
            else {
      ...
      while ( it.hasNext() ) {
         f = (Frame)it.next();
         pic.addFrame( f );
         f.getSettings().adjust( dimension.width, dimension.height );
      }
   ...
   public void addFrame( Frame aFrame ) {
      frames.add( aFrame );
      notifyDataListeners( new ChangeEvent( this, PICTURE_FRAME_ADDED, getFrameCount() - 1 ) );
   }
   ...
   // bringt die paletten auf eine einheitliche größe
...
//   }
   ...
   public int getPictureBackground() {
      if ( currentFrame != -1 ) {
         final int maxColours = getCurrentFrame().getPalette().size();
         if ( pictureBackground >= maxColours ) {
            return 0;
         }
      }
   ...
   public int reduceColours( int maxColours ) {
      ...
      final int palCount = pals.length;
      for ( int i = 0; i < palCount; i++ ) {
         pals[i].reducePalette( maxColours );
      }
      allowGUIEvents( this );
   ...
   public void removeFrame( final int index ) {
      ...
      f.detach();
      notifyDataListeners( new ChangeEvent( Picture.this, PICTURE_FRAME_REMOVED, index ) );
   }
   ...
   public void setBackground( int i ) {
      if ( i != background ) {
         int old = background;
         background = i;
         notifyDataListeners( new ChangeEvent( this, PICTURE_BACKGROUND_COLOUR_CHANGED, old, background ) );
      }
   }
   ...
   public void setCurrentFrame( int index ) {
      if ( index != currentFrame ) {
         ...
         notifyDataListeners( new ChangeEvent( this, PICTURE_FRAME_SET, old, currentFrame ) );
      }
   }
   ...
   public void setForeground( int i ) {
      if ( i != foreground ) {
         int old = foreground;
         foreground = i;
         notifyDataListeners( new ChangeEvent( this, PICTURE_FOREGROUND_COLOUR_CHANGED, old, foreground ) );
      }
   }
   ...
   public void setPictureBackground( int index ) {
      if ( index != pictureBackground ) {
         ...
         notifyDataListeners( new ChangeEvent( this, PICTURE_MAIN_BACKGROUND_CHANGED, old, index ) );
      }
   }
