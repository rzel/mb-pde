...
public final class ColourEntry extends DataObject implements Serializable, Comparable {
   ...
   public ColourEntry copy() {
      ColourEntry e = new ColourEntry( colour, index );
      return e;
   }
   ...
   public void removeAlpha() {
      ...
      notifyDataListeners( new ChangeEvent( this, ENTRY_RGBA_CHANGED, oldCol, newCol ) );
   }
   ...
   public boolean isOpaque() {
      return (!isTransparent) && (colour.getAlpha() == OPAQUE);
   }
   ...
   public boolean isSimilar( ColourEntry e, int diff ) {
      ...
      int aDiff = Math.abs( getAlpha() - e.getAlpha() );

      return (rDiff <= diff) && (gDiff <= diff) && (bDiff <= diff) && (aDiff <= diff);
   }

   protected void setIndexQuiet( int anIndex ) {
      index = anIndex;
   }
   ...
   public void setIndex( int anIndex ) {
      ...
      index = anIndex;
      if ( index == -1 ) {
         notifyDataListeners( new ChangeEvent( this, ENTRY_DISPOSED, oldIndex, index ) );
      }
      else {
         notifyDataListeners( new ChangeEvent( this, ENTRY_REINDEXED, oldIndex, index ) );
      }
   }
   ...
   protected void setColour( Color c ) {
      ...
      if ( isTransparent && (c.getAlpha() != OPAQUE) ) {
         Log.warning( "The alpha value of the palette's transparent colour mustn't "
               + "be changed.\n\n<font color=blue><b>Hint</b></font>: "
               + "Ctrl-left-click this colour to remove its "
               + "transparent colour status. The alpha value can " + "then be modified." );
      }
   }
   ...
   public int compareTo( Object o ) {
      ...
      float[] ehsb = Color.RGBtoHSB( e.colour.getRed(), e.colour.getGreen(), e.colour.getBlue(), null );
   ...
   protected static final String getColourString( int red, int green, int blue, int alpha ) {
      StringBuffer buf = new StringBuffer();
      buf.append( red );
      buf.append( '.' );
      buf.append( green );
